(function () {
  ...
  var parse_1 = parse$2.parsePatch;
  ...
  /*istanbul ignore end*/function applyPatch(source, uniDiff) {
    ...
    if (typeof uniDiff === 'string') {
      uniDiff = /*istanbul ignore start*/(0, parse$2.parsePatch) /*istanbul ignore end*/(uniDiff);
    }
  ...
  function loadPatch(param, base) {
    if (typeof param === 'string') {
      if (/^@@/m.test(param) || /^Index:/m.test(param)) {
        return (/*istanbul ignore start*/(0, parse$2.parsePatch) /*istanbul ignore end*/(param)[0]
        );
      }
  ...
  /*istanbul ignore start*/exports. /*istanbul ignore end*/parsePatch = parse$2.parsePatch;
  ...
  var diffPrettyHtml = function (text1, text2) {
      var changes = lib_11(text1.split('\n'), text2.split('\n'));
  ...
  var pprintAssertionErrorHtml = function (e) {
      var dh = diffPrettyHtml(e.diff.actual, e.diff.expected);
      return "Test failure: " + e.message + "\nExpected: \n" + htmlentities(e.diff.expected) + "\nActual: \n" + htmlentities(e.diff.actual) + "\nDiff: \n" + dh;
  };
  var pprintAssertionText = function (e) {
      var dh = diffPrettyText(e.diff.actual, e.diff.expected);
      return "Test failure: " + e.message + "\nExpected: \n" + e.diff.expected + "\nActual: \n" + e.diff.actual + "\nDiff: \n" + dh;
  };
  ...
  var params$1 = UrlParams.parse(window.location.search, makeSessionId);
  ...
  var summary = function () { return ({
      passed: passCount + (params$1.offset - params$1.failed),
      failed: failCount + params$1.failed,
  }); };
  var reporter = (function () {
      var current = $('<span />').addClass('progress').text(params$1.offset);
      ...
      var retryBtn = $('<button />').text('Retry').on('click', function () {
          var sum = summary();
          var url = makeUrl(params$1.session, sum.passed + sum.failed - 1, sum.failed - 1, 0);
          window.location.assign(url);
      }).hide();
      var skipBtn = $('<button />').text('Skip').on('click', function () {
          var sum = summary();
          var url = makeUrl(params$1.session, sum.passed + sum.failed, sum.failed, 0);
          window.location.assign(url);
      }).hide();
      $(function () {
          $('body')
      ...
      var keepAliveTimer = setInterval(function () {
          sendKeepAlive(params$1.session, noop, function () {
      ...
      var test = function (file, name) {
          ...
          var start = function (onDone) {
              ...
              $('body').append(el);
              reported = false;
              sendTestStart(params$1.session, file, name, onDone, onDone);
          };
          var pass = function (onDone) {
              ...
              time.text(testTime);
              current.text(params$1.offset + passCount + failCount);
              sendTestResult(params$1.session, file, name, true, testTime, null, onDone, onDone);
          };
          var fail = function (e, onDone) {
              ...
              sendTestResult(params$1.session, file, name, false, testTime, textError, onDone, onDone);
          };
          return {
              start: start,
              pass: pass,
      ...
      var done = function () {
          ...
          sendDone(params$1.session, setAsDone, setAsDone);
      };
  ...
  var runGlobalTests = function () {
      var loadNextChunk = function () {
          if (globalTests.length > (params$1.offset + chunk)) {
              var url = makeUrl(params$1.session, params$1.offset + chunk, reporter.summary().failed, 0);
              window.location.assign(url);
          }
          else {
              reporter.done();
              // for easy rerun reset the URL
              window.history.pushState({}, '', makeUrl(params$1.session, 0, 0, 0));
          }
      };
      var retryTest = function () {
          var sum = reporter.summary();
          window.location.assign(makeUrl(params$1.session, sum.passed + sum.failed - 1, sum.failed - 1, params$1.retry + 1));
      };
      var loadNextTest = function () {
          var sum = reporter.summary();
          window.location.assign(makeUrl(params$1.session, sum.passed + sum.failed, sum.failed, 0));
      };
      var afterFail = function () {
          ...
          else if (params$1.retry < retries) {
              retryTest();
          }
          else {
              loadNextTest();
                  ...
                  report_1.start(function () {
                      test.test(function () {
                          ...
                          report_1.pass(function () {
                              if (params$1.retry > 0) {
                                  params$1.retry = 0;
                                  var url = makeUrl(params$1.session, params$1.offset, params$1.failed, params$1.retry);
                                  window.history.pushState({}, '', url);
                              }
                              loop(tests);
                          });
      ...
      loop(globalTests.slice(params$1.offset, params$1.offset + chunk));
  };
