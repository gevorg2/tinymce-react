...
UnitTest.test('Check that if the filter always returns false, then everything is in "f"', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(), fc.string(1, 40)), function (obj) {
...
UnitTest.test('Check that if the filter always returns true, then everything is in "t"', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(), fc.string(1, 40)), function (obj) {
...
UnitTest.test('Check that everything in f fails predicate and everything in t passes predicate', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(1, 30), fc.integer()), function (obj) {
        var predicate = function (x) { return x % 2 === 0; };
        var output = Obj.bifilter(obj, predicate);
        var matches = function (k) { return predicate(obj[k]); };
