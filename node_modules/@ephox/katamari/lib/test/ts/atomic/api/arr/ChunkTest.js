...
UnitTest.test('chunk: unit tests', function () {
...
UnitTest.test('Chunking should create an array of the appropriate length except for the last one', function () {
    fc.assert(fc.property(fc.array(fc.integer()), fc.nat(), function (arr, rawChunkSize) {
        // ensure chunkSize is at least one
        var chunkSize = rawChunkSize + 1;
        var chunks = Arr.chunk(arr, chunkSize);
        var hasRightSize = function (part) { return part.length === chunkSize; };
