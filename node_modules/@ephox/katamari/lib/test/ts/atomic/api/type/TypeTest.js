...
UnitTest.test('Type.is*: Unit tests', function () {
    var check = function (method, methodName) { return function (expected, input) {
        var actual = method(input);
        Assert.eq(function () { return 'Expected: ' + methodName + '(' + Pprint.render(input, Pprint.pprintAny) + ') to be ' + expected; }, expected, actual);
    }; };
    // tslint:disable-next-line:no-construct
    var objectString = new String('ball');
    var noop = function () { };
    var checkIsNull = check(Type.isNull, 'isNull');
    var checkIsUndefined = check(Type.isUndefined, 'isUndefined');
    var checkIsBoolean = check(Type.isBoolean, 'isBoolean');
    var checkIsString = check(Type.isString, 'isString');
    var checkIsObject = check(Type.isObject, 'isObject');
    var checkIsArray = check(Type.isArray, 'isArray');
    var checkIsFunction = check(Type.isFunction, 'isFunction');
    var checkIsNumber = check(Type.isNumber, 'isNumber');
...
UnitTest.test('Type.is*: only one should match for every value', function () {
    var classifiers = [
        Type.isString,
        Type.isObject,
        Type.isArray,
    ...
    fc.assert(fc.property(fc.anything(), function (x) {
        var matches = Arr.filter(classifiers, function (c) { return c(x); });
