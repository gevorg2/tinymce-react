...
UnitTest.test('the value found by find always passes predicate', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(), fc.json()), fc.func(fc.boolean()), function (obj, pred) {
...
UnitTest.test('If predicate is always false, then find is always none', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(), fc.json()), function (obj) {
        var value = Obj.find(obj, Fun.constant(false));
        return value.isNone();
    }));
});
UnitTest.test('If object is empty, find is always none', function () {
    fc.assert(fc.property(fc.func(fc.boolean()), function (pred) {
        var value = Obj.find({}, pred);
        return value.isNone();
    }));
});
UnitTest.test('If predicate is always true, then value is always the some(first), or none if dict is empty', function () {
    fc.assert(fc.property(fc.dictionary(fc.asciiString(), fc.json()), function (obj) {
