...
UnitTest.test('Length of interspersed = len(arr) + len(arr)-1', function () {
    fc.assert(fc.property(fc.array(fc.integer()), fc.integer(), function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
        var expected = arr.length === 0 ? 0 : arr.length * 2 - 1;
        Assert.eq('eq', expected, actual.length);
    }));
});
UnitTest.test('Every odd element matches delimiter', function () {
    fc.assert(fc.property(fc.array(fc.integer()), fc.integer(), function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
        return Arr.forall(actual, function (x, i) { return i % 2 === 1 ? x === delimiter : true; });
    }));
});
UnitTest.test('Filtering out delimiters (assuming different type to array to avoid removing original array) should equal original', function () {
    fc.assert(fc.property(fc.array(fc.nat()), arbNegativeInteger(), function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
        var filtered = Arr.filter(actual, function (a) { return a !== delimiter; });
