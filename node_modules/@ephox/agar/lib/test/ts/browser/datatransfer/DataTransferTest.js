...
UnitTest.test('DataTransfer: setEffects', function () {
...
UnitTest.test('DataTransfer: setData', function () {
    var transfer = createDataTransfer();
...
UnitTest.test('DataTransfer: setDragImage', function () {
    var transfer = createDataTransfer();
    KAssert.eqNone('Should not have a drag image', getDragImage(transfer));
    transfer.setDragImage(document.createElement('div'), 10, 20);
    KAssert.eqSome('Should be expected element', 'DIV', getDragImage(transfer).map(function (x) { return x.image.nodeName; }));
    KAssert.eqSome('Should be expected x cord', 10, getDragImage(transfer).map(function (x) { return x.x; }));
    KAssert.eqSome('Should be expected y cord', 20, getDragImage(transfer).map(function (x) { return x.y; }));
});
UnitTest.test('DataTransfer: testTypes', function () {
    var transfer = createDataTransfer();
    transfer.setData('text/plain', '123');
...
UnitTest.test('DataTransfer: mutation in protected mode', function () {
    var transfer = createDataTransfer();
    transfer.setData('text/html', '123');
...
UnitTest.test('DataTransfer: mutation in read-only mode', function () {
    var transfer = createDataTransfer();
    transfer.setData('text/html', '123');
    transfer.items.add(createFile('test.gif', 123, new Blob([''], { type: 'image/gif' })));
...
UnitTest.test('DataTransfer: add files', function () {
    var transfer = createDataTransfer();
    transfer.items.add(createFile('test.gif', 123, new Blob([''], { type: 'image/gif' })));
    Assert.eq('Should be able to access files length', 1, transfer.files.length);
    Assert.eq('Types', ['image/gif'], Arr.map(transfer.files, function (x) { return x.type; }));
    transfer.items.add(createFile('test.jpg', 123, new Blob([''], { type: 'image/jpg' })));
    Assert.eq('Expected file types', ['image/gif', 'image/jpg'], Arr.map(transfer.files, function (x) { return x.type; }));
    Assert.eq('Expected file kinds', ['file', 'file'], Arr.map(transfer.items, function (x) { return x.kind; }));
});
