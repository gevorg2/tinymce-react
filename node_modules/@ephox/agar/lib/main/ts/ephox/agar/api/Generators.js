...
var selection = function (container, exclusions) { return GenSelection.selection(container, exclusions); };
var describeSelection = function (root, generated) {
    return Hierarchy.path(root, generated.start()).bind(function (startPath) {
        return Hierarchy.path(root, generated.finish()).map(function (finishPath) { return ({
            selection: {
...
var chooseOne = function (choices) { return TagDecorator.gOne(choices); };
var enforce = function (attrs) { return TagDecorator.gEnforce(attrs); };
...
var hexColor = Jsc.tuple([
    ...
    hexDigit,
    hexDigit
]).generator.map(function (digits) { return ['#'].concat(digits).join(''); });
