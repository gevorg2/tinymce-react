...
import { Node, PredicateFilter, Text, Traverse } from '@ephox/sugar';
...
var gChooseIn = function (target) {
    ...
    return Jsc.integer(0, offsets).generator.map(function (offset) { return ({ element: target, offset: offset }); });
};
...
var selection = function (root, rawExclusions) {
    ...
    return gChooseFrom(root, exclusions).flatMap(function (start) { return gChooseFrom(root, exclusions).map(function (finish) { return ({
