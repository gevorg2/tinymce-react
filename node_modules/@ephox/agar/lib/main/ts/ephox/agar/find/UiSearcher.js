...
var selectAll = function (element, selector) {
    return derive(element, selector).fold(function (element, selector) { return matchesSelf(element, selector).toArray(); }, SizzleFind.children, SizzleFind.descendants);
};
var toResult = function (message, option) {
    return option.fold(function () { return Result.error(message); }, Result.value);
};
var findIn = function (container, selector) {
    return toResult(function () { return 'Could not find selector: ' + selector + ' in ' + Truncate.getHtml(container); }, select(container, selector));
};
