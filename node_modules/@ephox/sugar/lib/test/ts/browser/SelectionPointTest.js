...
UnitTest.asynctest('Browser Test: Selection.getAtPoint', function (success, failure) {
    ...
    var iframe = Element.fromHtml('<iframe style="position: fixed; top: 0; left: 0; height:700px; width:700px;" src="/project/@ephox/sugar/src/test/data/points.html"></iframe>');
    ...
    var checks = function () {
        ...
        var check = function (x, y, expected) {
            ...
            assert.eq(true, starts, function () { return 'start elements were not equal, was ' + Node.name(range.start()) + ', expected ' + Node.name(expected.start()); });
            assert.eq(expected.soffset(), range.soffset());
            assert.eq(true, Compare.eq(expected.finish(), range.finish()), function () { return 'finish elements were not equal, was ' + Node.name(range.finish()); });
