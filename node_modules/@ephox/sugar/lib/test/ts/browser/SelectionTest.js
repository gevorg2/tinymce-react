...
UnitTest.test('Browser Test: SelectionTest', function () {
    ...
    var assertSelection = function (label, expStart, expSoffset, expFinish, expFoffset) {
        WindowSelection.getExact(window).fold(function () {
            ...
            assert.eq(true, Compare.eq(sel.start(), expStart), function () { return 'Start container should be: ' + Html.getOuter(expStart) + '\n' + label; });
            assert.eq(expSoffset, sel.soffset());
            assert.eq(true, Compare.eq(sel.finish(), expFinish), function () { return 'Finish container should be ' + Html.getOuter(expFinish) + '\n' + label; });
    ...
    var assertRng = function (selection, expStart, expSoffset, expFinish, expFoffset) {
        ...
        assert.eq(expStart.dom(), r.startContainer, function () { return 'Start Container should be: ' + Html.getOuter(expStart); });
        assert.eq(expSoffset, r.startOffset, 'Start offset should be: ' + expSoffset);
        assert.eq(expFinish.dom(), r.endContainer, function () { return 'End Container should be: ' + Html.getOuter(expFinish); });
